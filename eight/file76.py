...
def get_NP_feature():
	...
	for sentense_lists_list in get_dic_list(sys.argv[1]):
		wf_number += 1
		wf_str = "learning_data/data0%s.f" % wf_number
		wf = open(wf_str, 'w')
		for sentense_list in sentense_lists_list: #english_1.txt.geniaとかのそれぞれに対応したリストがsentense_listである
		    BNP_flag = False
		    INP_flag = False
		    before_word = ""
		    before_pos = ""
		    ...
		    if after_word_flag:
		    	after_word_flag = False
		    	wf.write(feature_str + '\tw[1]=NONE\tpos[1]=NONE\n')
		    ...
		    for token_dic in sentense_list:
		        ...
		        if after_word_flag:
		        	after_word_flag = False
		        	wf.write(feature_str + '\tw[1]=%s\tpos[1]=%s\n' % (token_dic["w"],token_dic["pos"]))
